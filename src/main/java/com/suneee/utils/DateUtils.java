...
public class DateUtils extends org.apache.commons.lang.time.DateUtils {
     ...
     * 解决多线程带来的影响
     *
     * @param df 自己传入 format的格式
     * @return 返回 yyyy-MM-dd HH:mm:ss 格式
     */
    public static final DateFormat getDateFormat(String df) {

        DateFormat format = messageFormat.get();
        if (format == null) {
            format = new SimpleDateFormat(df, Locale.getDefault());
    ...
    /**
     * 计算时间差值（单位为毫秒）(采用ThreadLocal避免多线程影响)
     *
     * @param time1 时间1
     * @param time2 时间2
     * @return 差值
    ...
    public static long minus(String time1, String time2) {
        try {
            Date datetime1 = getDateFormat().parse(time1);
            Date datetime2 = getDateFormat().parse(time2);
    ...
    public static Date StringToDate(String dateString) {
        ...
        try {
            ...
            logger.error("StringToDate fail", e);
        }
        return date;
    }
     ...
     * @param _start
     * @param _end
     * @return 为Date类型, 根据需要自己format
     * @throws ParseException
     * @Description: 支持传入日期 格式（yyyy-mm-dd 或 yyyyMMdd）
     */
    public static List<Date> findDates(String _start, String _end) throws ParseException {
            _start =   _start.replaceAll("-", "");
            _end = _end.replaceAll("-", "");

//        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
        Date dBegin = SHORTDATEFORMAT.parse(_start);
        Date dEnd = SHORTDATEFORMAT.parse(_end);
